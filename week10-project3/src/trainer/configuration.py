...
class SchedulerConfig:
    scheduler: str = "MultiStepLR"   # MultiStepLR, Cosine, "ReduceLROnPlateau",
    lr_step_milestones: Iterable = (
        30, 40
    )  # at which epoches should we make a "step" in learning rate (i.e. decrease it in some manner)
    lr_gamma     : float = 0.1  # multiplier applied to current learning rate at each of lr_ctep_milestones
    sch_factor   : float  = 0.75
    sch_patience : int = 5
    sch_cooldown : int = 3
...
class TrainerConfig:
    ...
    session_name: str = ''      # name of main python program or notebook
    project_name: str = ''      # project name (used for weights and biases )
