...
def train_hook_default(
    ...
    for i, sample in enumerate(iterator):
        ...
        status = f"{prefix}[Train][{i}] Loss_avg: {loss_avg.avg:.5}, Loss: {loss_avg.val:.5}, LR: {optimizer.param_groups[0]["lr"]:e}"
            # .format(prefix, i, loss_avg.avg, loss_avg.val, optimizer.param_groups[0]["lr"])
...
def train_hook_detection(
    ...
    """
    model = model.train()
    iterator = iterator_type(loader, disable=not stage_progress, dynamic_ncols=True, leave = False)
    loss_avg = AverageMeter()
    loc_loss_avg = AverageMeter()
    ...
    for i, sample in enumerate(iterator):
        ...
        loc_loss_avg.update(loc_loss.item())
        cls_loss_avg.update(cls_loss.item())
        loss_avg.update(loss.item())
        
        status = f"{prefix}[Train][{i:2d}] Loss_avg: {loss_avg.avg:.5}, Loss: {loss_avg.val:.5}, Loc_loss: {loc_loss:.5}, Cls_loss: {cls_loss:.5}, LR: {optimizer.param_groups[0]["lr"]:.2e}"
            # .format(prefix, i, loss_avg.avg, loss_avg.val, loc_loss, cls_loss, optimizer.param_groups[0]["lr"])
        iterator.set_description(status)
    return {"loss": loss_avg.avg , "loc_loss": loc_loss_avg,  "cls_loss": cls_loss_avg,}
...
def test_hook_detection(
    ...
    for i, sample in enumerate(iterator):
        ...
        for img_in_batch in range(0, batch_size):
            _, boxes, labels, _ = loader.dataset[i + img_in_batch]
        ...
        status = f"{prefix}[Val][{i:2d}] Loss_avg: {loss_avg.avg:.5}, Loss: {loss_avg.val:.5}, Loc_loss: {loc_loss:.5}, Cls_loss: {cls_loss:.5} "
        iterator.set_description(status)
    ...
    output = {"metric": metric_fn.get_metric_value(), "loss": loss_avg.avg, "loc_loss": loc_loss_avg,  "cls_loss": cls_loss_avg,}
    return output


def end_epoch_hook_detection(iterator, epoch, output_train, output_test, wandb_session, best_acc):
    ...
    if wandb_session:
        wandb_session.log({'trn_loss' : output_train["loss"], 
                           'val_loss' : output_test["loss"], 
                           'val_acc'  : output_test["metric"]["mAP"],
                           'best_acc' : best_acc,
                           # 'trn_acc'  : train_acc, 
                           # 'best_loss': metrics.best_loss, 
                           # 'best_loss_ep': metrics.best_loss_ep, 
                           # 'best_acc_ep' : metrics.best_acc_ep,
